...
import (
	...
	"github.com/Vioneta/VionetaOS-CLI/codegen/vionetaos"
...
var healthcheckLogsCmd = &cobra.Command{
	Use:     "logs",
	Short:   "get all `vionetaos-*` logs and save to a ZIP file",
	Aliases: []string{"log"},
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		url := fmt.Sprintf("http://%s/%s", rootURL, BasePathVionetaOS)

		client, err := vionetaos.NewClientWithResponses(url)
		if err != nil {
			return err
		}
		...
		if response.StatusCode() != http.StatusOK {
			var baseResponse vionetaos.BaseResponse
		...
		if outDir == "" {
			outDir, err = os.MkdirTemp("", "vionetaos-cli-*")
			if err != nil {
				return err
			}
		}

		zipFilePath := fmt.Sprintf("%s/vionetaos-%s-logs-%s.zip", outDir, Version, time.Now().Format("20060102150405"))
